...
export const ko: Dict = {
  ...
  'pluginDetails.viewDetailsAria': '{title} 세부 정보 보기',
  'pluginDetails.chooseUseAria': '{title} 사용 방식 선택',
  'pluginDetails.useOptionsAria': '{title} 사용 옵션',
  'pluginDetails.shareAria': '{title} 공유',
  'pluginDetails.publishGithubAria': '{title}를 GitHub 저장소로 게시',
  'pluginDetails.contributeOpenDesignAria': '{title}를 Open Design에 기여',
  'pluginDetails.publishGithub': '플러그인을 GitHub 저장소로 게시',
  'pluginDetails.contributeOpenDesign': '풀 리퀘스트로 Open Design에 기여',
  'pluginDetails.usePlugin': '사용',
  'pluginDetails.useWithQuery': '쿼리와 함께 사용',
  'pluginDetails.publish': '게시',
  'pluginDetails.contribute': '기여',
  'pluginDetails.starting': '시작 중…',
  'pluginDetails.pluginInfo': '플러그인 정보',

  'homeHero.title': '무엇을 디자인할까요?',
  'homeHero.subtitlePrefix': '아래 플러그인을 선택해 예시 쿼리를 불러오거나, 자유롭게 입력한 뒤',
  'homeHero.placeholder': '디자인 요청을 설명하거나 파일을 붙여넣고, 플러그인·스킬·MCP·커넥터를 검색하세요…',
  'homeHero.placeholderActive': '예시 쿼리를 수정하거나 원하는 내용을 입력하세요…',
  'homeHero.skills': '스킬',
  ...
  'homeHero.confirmReplaceBody': '{title}을 사용하면 입력창의 기존 텍스트가 대체됩니다.',
  'homeHero.confirmReplace': '대체',
  ...
  'homeHero.chip.hyperframes': '하이퍼프레임',
  'homeHero.chip.audio': '오디오',
  'homeHero.chip.createPlugin': '플러그인 만들기',
  'homeHero.chip.figma': 'Figma에서',
  'homeHero.chip.folder': '폴더에서',
  'homeHero.chip.template': '템플릿에서',
  'homeHero.chip.liveArtifactHint': '미리보기 가능한 인터랙티브 HTML/CSS/JS 아티팩트를 만듭니다.',
  'homeHero.chip.hyperframesHint': '캡션, 오디오 반응형 비주얼, 장면 전환 등 HTML 기반 모션을 만듭니다.',
  'homeHero.chip.createPluginHint': '재사용 가능한 Open Design 플러그인을 만들고 내 플러그인에 추가합니다.',
  'homeHero.chip.figmaHint': 'Figma 프레임을 현재 디자인 시스템으로 마이그레이션합니다.',
  'homeHero.chip.folderHint': '기존 로컬 폴더를 가져와 계속 편집합니다.',
  'homeHero.chip.templateHint': '번들 템플릿에서 시작합니다.',
  'pluginsHome.title': '공식 스타터',
  'pluginsHome.subtitle': '이 런타임에 포함된 바로 사용 가능한 Open Design 워크플로입니다. 스타터 프롬프트를 불러오거나 레지스트리에서 더 찾아보세요.',
  'pluginsHome.browseRegistry': '레지스트리 보기',
  'pluginsHome.count': '{total}개 중 {filtered}개',
  'pluginsHome.loadingCatalog': '카탈로그 불러오는 중…',
  'pluginsHome.emptyCatalog': '카탈로그가 비어 있습니다. 번들 플러그인은 자동으로 표시되어야 하므로 계속 비어 있으면 daemon을 재시작하세요.',
  'pluginsHome.emptyFiltered': '현재 필터와 일치하는 플러그인이 없습니다.',
  'pluginsHome.clearFilters': '필터 지우기',
  'pluginsHome.contributeTitle': '{label} 플러그인 기여',
  'pluginsHome.contributeBody': '아직 항목이 적은 영역입니다. 워크플로를 재사용 가능한 플러그인으로 만들고 내 플러그인에 추가한 뒤 커뮤니티에 공유하세요.',
  'pluginsHome.starterPrefix': '스타터: {starter}',
  'pluginsHome.modeAria': '플러그인 모드',
  'pluginsHome.featured': '추천',
  'pluginsHome.totalInCatalog': '카탈로그 {n}개',
  ...
  'promptTemplates.sortAria': '템플릿 정렬',
  'promptTemplates.sortName': '이름순',
  'promptTemplates.sortNewest': '최신순',
  'promptTemplates.newBadge': '신규',
  ...
  'chat.comments.placeholder': '이 요소에 대한 댓글을 입력하세요…',
  ...
  'chat.comments.pinSavedToast': '고정 저장됨',
  'chat.comments.pinAtCoords': '{x}, {y} 위치',
  'chat.comments.capturedItems': '{n}개 항목 캡처됨',
  ...
  'liveArtifact.refresh.noSourceTitle': '아직 새로고침할 소스가 없습니다.',
  ...
  'liveArtifact.refresh.failureAction': '새로고침 기록을 확인하고 소스나 권한 문제를 해결한 뒤 다시 시도하세요.',
  'liveArtifact.refresh.networkFailure': '새로고침 요청에 실패했습니다. 연결 상태를 확인한 뒤 다시 시도하세요.',
  'liveArtifact.refresh.genericFailure': '새로고침 실패.',
  'liveArtifact.refresh.statusNever': '새로고침할 수 없음',
...
function applyKoreanFallbacks(): void {
  for (const key of Object.keys(en) as Array<keyof Dict>) {
    if (ko[key] !== en[key]) continue;
    ko[key] = koreanFallbackCopy(key, en[key]);
  }
}

function koreanFallbackCopy(key: keyof Dict, english: string): string {
  const placeholders = Array.from(english.matchAll(/\{(\w+)\}/g), (match) => match[0]);
  const [namespace = '', ...rest] = String(key).split('.');
  const leaf = rest[rest.length - 1] ?? namespace;
  const namespaceLabel = namespaceLabels[namespace] ?? '항목';
  const leafLabel = leafLabels[leaf] ?? splitKeyLabel(leaf);
  const suffix = placeholders.length > 0 ? ` ${placeholders.join(' ')}` : '';
  return `${namespaceLabel} ${leafLabel}${suffix}`;
}

function splitKeyLabel(value: string): string {
  const words = value
  ...
  return translated.length > 0 ? translated.join(' ') : '문구';
}
